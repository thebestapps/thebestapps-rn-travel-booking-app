    ...
    <View style={style.flights}>
      ...
      <GestureHandlerRootView>
        <PanGestureHandler onGestureEvent={animatedGestureHandler}>
          <Animated.View style={[style.flights__swipBtn, AnimatedStyles.swipeable]} />
        </PanGestureHandler>
        <View style={[style.flights__modeSwitcher, {marginTop: 20}]}>
          <Text 
            style={{color: (mode === 0) ? Colors.blackText : Colors.subText }} 
            onPress={() => {Z.value = withSpring(0, {damping: 20, stiffness: 500}); setMode(0)}}>
              One-way
          </Text>
          <Text 
            style={{color: (mode === 1) ? Colors.blackText : Colors.subText }} 
            onPress={() => {Z.value = withSpring(117, {damping: 20, stiffness: 500}); setMode(1)}}>
              Round trip
          </Text>
          <Text 
            style={{color: (mode === 2) ? Colors.blackText : Colors.subText }} 
            onPress={() => {Z.value = withSpring(235, {damping: 20, stiffness: 500}); setMode(2)}}>
              Multi-city
          </Text>
        </View>
      </GestureHandlerRootView>
